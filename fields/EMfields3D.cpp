  ...
  Jz_ext(nxn,nyn,nzn) 
{
  ...
  for(int i=0;i<sizeMomentsArray;i++)
  {
    ...
    MPI_Type_create_hvector((nxc-2),(nzc-2),static_cast<MPI_Aint>(nzc*nyc*sizeof(double)), MPI_DOUBLE, &xzFacetypeC);
    ...
    MPI_Type_create_hvector((nxc-2), 1, static_cast<MPI_Aint>(nzc*nyc*sizeof(double)), yEdgetypeC, &xyFacetypeC);
    MPI_Type_commit(&xyFacetypeC);
    
    //2 yEdgeType can be merged into one message
    MPI_Type_create_hvector(2, 1,static_cast<MPI_Aint>((nzc-1)*sizeof(double)), yEdgetypeC, &yEdgetypeC2);
    MPI_Type_commit(&yEdgetypeC2);
    
    MPI_Type_contiguous((nzc-2),MPI_DOUBLE, &zEdgetypeC);
    MPI_Type_commit(&zEdgetypeC);
    
    MPI_Type_create_hvector(2, (nzc-2),static_cast<MPI_Aint>((nxc-1)*(nyc*nzc)*sizeof(double)), MPI_DOUBLE, &zEdgetypeC2);
    MPI_Type_commit(&zEdgetypeC2);
    
    MPI_Type_vector((nxc-2), 1, nyc*nzc, MPI_DOUBLE, &xEdgetypeC);
    MPI_Type_commit(&xEdgetypeC);
    MPI_Type_create_hvector(2, 1, static_cast<MPI_Aint>((nyc-1)*nzc*sizeof(double)), xEdgetypeC, &xEdgetypeC2);
    ...
    //For face exchange on Y dir
    MPI_Type_create_hvector((nxn-2),(nzn-2),static_cast<MPI_Aint>(nzn*nyn*sizeof(double)), MPI_DOUBLE, &xzFacetypeN);
    ...
    //For face exchangeg on Z dir
    MPI_Type_create_hvector((nxn-2), 1, static_cast<MPI_Aint>(nzn*nyn*sizeof(double)), yEdgetypeN, &xyFacetypeN);
    MPI_Type_commit(&xyFacetypeN);

    //2 yEdgeType can be merged into one message
    MPI_Type_create_hvector(2, 1,static_cast<MPI_Aint>((nzn-1)*sizeof(double)), yEdgetypeN, &yEdgetypeN2);
    MPI_Type_commit(&yEdgetypeN2);

    MPI_Type_contiguous((nzn-2),MPI_DOUBLE, &zEdgetypeN);
    MPI_Type_commit(&zEdgetypeN);

    MPI_Type_create_hvector(2, (nzn-2),static_cast<MPI_Aint>((nxn-1)*(nyn*nzn)*sizeof(double)), MPI_DOUBLE, &zEdgetypeN2);
    MPI_Type_commit(&zEdgetypeN2);

    MPI_Type_vector((nxn-2), 1, nyn*nzn, MPI_DOUBLE, &xEdgetypeN);
    MPI_Type_commit(&xEdgetypeN);
    MPI_Type_create_hvector(2, 1, static_cast<MPI_Aint>((nyn-1)*nzn*sizeof(double)), xEdgetypeN, &xEdgetypeN2);
